...
export default function App() {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-dark-900 bg-gradient-dark-mesh flex items-center justify-center text-slate-100 font-sans">
        ...
        {slides.map((_, idx) => (
          <button
            ...
            className={`transition-all duration-300 rounded-full ${
              current === idx
                ? 'w-8 h-2 bg-primary shadow-[0_0_10px_rgba(16,185,129,0.8)]'
      ...
      <div
        className="absolute top-0 left-0 w-1/4 h-full z-40 cursor-w-resize"
        onClick={prevSlide}
      />
      <div
        className="absolute top-0 right-0 w-1/4 h-full z-40 cursor-e-resize"
        onClick={nextSlide}
      />
    </div>
  )
