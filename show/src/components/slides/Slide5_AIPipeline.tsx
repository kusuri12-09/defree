      ...
      <div className="w-full flex justify-between items-center mb-16 relative">
        ...
        {steps.map((step, idx) => (
          <div key={idx} className="flex items-center">
            ...
            >
              <div
                className={`w-24 h-24 rounded-full ${step.bg} border-2 border-white/20 flex items-center justify-center mb-4 backdrop-blur-md shadow-lg`}
              >
                <div className={`[&>svg]:w-10 [&>svg]:h-10 ${step.color}`}>{step.icon}</div>
              </div>
