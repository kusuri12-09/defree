...
export const Slide4_UXPrinciple = () => {
  const principles = [
    { icon: <User className="w-8 h-8 text-primary" />, title: '1인분 레시피 고정', desc: '모든 추천 레시피 분량을 1인용으로 자동 환산' },
    { icon: <Package className="w-8 h-8 text-[#3b82f6]" />, title: '소포장 우선 추천', desc: '장바구니 생성 시 낱개 상품 최상단 노출' },
    { icon: <BellDot className="w-8 h-8 text-[#f59e0b]" />, title: '조기 알림 시스템', desc: '1인 소비 속도를 고려한 D-3 선제적 알림' },
    { icon: <AlertTriangle className="w-8 h-8 text-danger" />, title: '소비 가능량 경고', desc: '"혼자 다 드시려면 3일간 3끼를 다 요리해야 해요."' },
  ];
