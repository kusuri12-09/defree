  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button className="text-primary text-sm font-bold bg-primary-light px-3 py-1 rounded-full">
          + 직접 추가
        </button>
      </div>

      <div className="flex space-x-2 border-b border-neutral-100 pb-2 overflow-x-auto">
        <button className="text-primary font-bold border-b-2 border-primary px-2 whitespace-nowrap">
          전체
        </button>
        ...
        <div className="space-y-3 pt-2">
          {data?.data?.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`p-3 rounded-xl border flex justify-between items-center shadow-sm ${isDanger ? 'bg-danger/5 border-danger/10' : 'bg-surface border-neutral-100'}`}
              >
                ...
                <span
                  className={`font-bold text-sm ${isDanger ? 'text-danger' : 'text-neutral-500'}`}
                >
                  {daysLeft > 0
                    ? `D-${daysLeft}`
                    : daysLeft === 0
