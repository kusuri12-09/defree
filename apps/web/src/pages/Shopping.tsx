  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="bg-primary-light p-4 rounded-xl border border-primary/20">
        ...
        <p className="text-md mt-1 font-bold text-neutral-900">"계란이 있으니 대파만 사세요!"</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {data.items.map((item) => (
            <label
              key={item.id}
              className="flex items-center p-3 bg-surface rounded-xl shadow-sm border border-neutral-100 cursor-pointer"
            >
              <input
                type="checkbox"
              ...
              <div className="ml-3 flex-1 flex justify-between items-center">
                <span
                  className={`font-medium ${item.isPurchased ? 'line-through text-neutral-400' : ''}`}
                >
                  {item.ingredientName}
                </span>
