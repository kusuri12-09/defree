    ...
    <div className="p-4 space-y-6">
      ...
      {user ? (
        ...
        <div className="bg-surface p-4 rounded-xl shadow-sm text-center py-8">
          ...
          <a href={`${import.meta.env.VITE_API_URL || 'http://localhost:3000/v1'}/auth/google/login`} className="bg-primary text-white px-6 py-2 rounded-full font-bold">
            Google 로그인
          </a>
        </div>
      )}
      ...
      {user && (
        <>
          ...
          <section>
            <button onClick={handleLogout} className="w-full text-danger bg-danger/10 py-3 rounded-xl font-bold">
              로그아웃
            </button>
          </section>
        </>
