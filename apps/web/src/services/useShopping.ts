...
import type { ApiResponse, ShoppingListDto, CreateShoppingItemDto, ShoppingListItemDto } from '@defree/shared-types';
...
export const useAddShoppingItem = () => {
  ...
  return useMutation({
    mutationFn: async (payload: CreateShoppingItemDto) => {
      const { data } = await api.post<ApiResponse<ShoppingListItemDto>>('/shopping-list/items', payload);
      return data.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['shopping-list'] });
