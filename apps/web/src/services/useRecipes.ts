...
import type { ApiResponse, CreateCookingLogDto, CookingLogResponseDto, RecipeRecommendationsDto } from '@defree/shared-types';

export const useRecipeRecommendations = () => {
  return useQuery({
    queryKey: ['recipes', 'recommendations'],
    queryFn: async () => {
      const { data } = await api.get<ApiResponse<RecipeRecommendationsDto>>('/recipes/recommendations');
      return data.data;
    },
  });
};
